...
`ifndef SYNTHESIS
    always @(posedge cpu_clk) begin
        if (cpu_rstn) begin
            if ((state0 == ST_MISS_REFILL) && !burst0)
                $fatal(1, "[L2DCache] lane0 non-burst entered refill state");
            if ((state1 == ST_MISS_REFILL) && !burst1)
                $fatal(1, "[L2DCache] lane1 non-burst entered refill state");
            if ((drain_pending_count != 4'd0) && !drain_active)
                $fatal(1, "[L2DCache] write completions pending without active source");
            if (mem_store_wdone && drain_active &&
                (drain_pending_count == 4'd0) && !drain_fire)
                $fatal(1, "[L2DCache] write completion without a drain owner");
        end
    end
`endif
...
`ifndef SYNTHESIS
    always @(posedge cpu_clk) begin
        if (cpu_rstn) begin
            if ((state0 == ST_MISS_REFILL) && !burst0)
                $fatal(1, "[L2DCacheBram64] lane0 non-burst entered refill");
            if ((state1 == ST_MISS_REFILL) && !burst1)
                $fatal(1, "[L2DCacheBram64] lane1 non-burst entered refill");
            if (data_wr0_en && data_wr1_en &&
                (data_wr0_way == data_wr1_way) &&
                (data_wr0_bank == data_wr1_bank) &&
                (data_wr0_row == data_wr1_row))
                $fatal(1, "[L2DCacheBram64] dual write collision");
            if (meta_write_a && meta_write_b &&
                (meta_addr_a == meta_addr_b))
                $fatal(1, "[L2DCacheBram64] metadata dual write collision");
            if ((drain_pending_count != 4'd0) && !drain_active)
                $fatal(1, "[L2DCacheBram64] write completions pending without owner");
            if (mem_store_wdone && !bypass_pending &&
                !(drain_active &&
                  ((drain_pending_count != 4'd0) || drain_fire)))
                $fatal(1, "[L2DCacheBram64] lower write completion without owner");
        end
    end
