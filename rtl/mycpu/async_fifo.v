...
`ifndef SYNTHESIS
    initial begin
        if (STAGES < 2)
            $error("ResetDomainSync requires STAGES >= 2");
    end
`endif
...
`ifndef SYNTHESIS
    initial begin
        if (FIFO_DEPTH < 16 || ((FIFO_DEPTH & (FIFO_DEPTH - 1)) != 0))
            $error("CdcAsyncFifo FIFO_DEPTH must be a power of two >= 16");
        if (SYNC_STAGES < 2)
            $error("CdcAsyncFifo SYNC_STAGES must be >= 2");
    end
`endif
