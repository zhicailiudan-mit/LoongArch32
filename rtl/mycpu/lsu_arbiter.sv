...
`ifndef SYNTHESIS
    longint unsigned tagged_cycle_count;
    longint unsigned tagged_max_owner;
    longint unsigned tagged_two_inflight;
    longint unsigned tagged_three_inflight;
    longint unsigned tagged_four_inflight;
    longint unsigned tagged_eight_inflight;
    longint unsigned tagged_max_cq;

    always_ff @(posedge clk or negedge rstn) begin
        if (!rstn) begin
            tagged_cycle_count <= 0;
            tagged_max_owner <= 0;
            tagged_two_inflight <= 0;
            tagged_three_inflight <= 0;
            tagged_four_inflight <= 0;
            tagged_eight_inflight <= 0;
            tagged_max_cq <= 0;
        end else begin
            tagged_cycle_count <= tagged_cycle_count + 1;
            if (owner_count > tagged_max_owner) tagged_max_owner <= owner_count;
            if (owner_count == 4'd2) tagged_two_inflight <= tagged_two_inflight + 1;
            if (owner_count == 4'd3) tagged_three_inflight <= tagged_three_inflight + 1;
            if (owner_count == 4'd4) tagged_four_inflight <= tagged_four_inflight + 1;
            if (owner_count == 4'd8) tagged_eight_inflight <= tagged_eight_inflight + 1;
            if (comp_count > tagged_max_cq) tagged_max_cq <= comp_count;
            if (tagged_cycle_count > 0 && (tagged_cycle_count % 100000 == 0))
                $display("[LSU-TAGGED-STATS] cycles=%0d max_owner=%0d two=%0d three=%0d four=%0d eight=%0d max_cq=%0d",
                         tagged_cycle_count, tagged_max_owner,
                         tagged_two_inflight, tagged_three_inflight,
                         tagged_four_inflight, tagged_eight_inflight,
                         tagged_max_cq);
        end
    end
    ...
    always @(posedge clk) begin
        if (rstn) begin
            if (owner_count > 4'd8 || comp_count > 4'd8 || total_credits_used > 5'd8)
                $fatal(1, "[ASSERT-LSU-TAGGED] TID/CQ credit bound violated");
            if (dcache_rsp.valid && !rsp_owner_valid)
                $fatal(1, "[ASSERT-LSU-TAGGED] response returned for a free TID");
            if ((|dcache_req.ren) && (|dcache_req.wen))
                $fatal(1, "[ASSERT-LSU-TAGGED] DCache request has Load and Store together");
            if (request_valid_q && request_kind_q == REQ_NONE)
                $fatal(1, "[ASSERT-LSU-TAGGED] request valid without kind");
        end
    end
